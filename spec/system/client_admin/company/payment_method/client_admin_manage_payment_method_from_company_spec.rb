...
describe 'Admin manage payment methods from company' do
  it 'successufuly' do
    PaymentMethod.create!(name: 'Nubank', payment_type: 0, bank_code: 260,
                          tax: 1.0, icon: fixture_file_upload(Rails.root.join('spec', 'fixtures', 'payment_method_icon.svg')))

    PaymentMethod.create!(name: 'Banco do Brasil', payment_type: 1,
                          bank_code: 0o01, tax: 5.0, icon: fixture_file_upload(Rails.root.join('spec', 'fixtures', 'payment_method_icon.svg')))

    PaymentMethod.create!(name: 'Inter', payment_type: 2, tax: 4.5,
                          bank_code: 0o77, icon: fixture_file_upload(Rails.root.join('spec', 'fixtures', 'payment_method_icon.svg')))
