class ClientsController < ApplicationController
    before_action :authenticate_user!,
	              only: %i[ index show ]
	before_action :set_client , only: [:show, :request_change_status]
	before_action :only_admin , only: [:index , :show, :request_change_status]
	...
	def request_change_status
		...
		if request.save
			redirect_to client_path(@client), notice: 'Aguardando autorização de outro Admin'
		else
	...
	def set_client
		@client = User.where("id = ? AND (role=? OR role=?)", params[:id], '0', '5' ).first
	end
