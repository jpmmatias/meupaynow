class CompanyPaymentsMethodsController < ApplicationController
  ...
  before_action :only_client_admin, only: %i[index new show]
  ...
  def create
    ...
    if @company_payment_method.save
      redirect_to company_company_payments_methods_path(@company),
                  notice: 'Método de Pagamento configurado com sucesso'
    else
      render :new
    end
  end
  ...
  def only_client_admin
    redirect_to dashboard_index_path unless current_user.role == 'client_admin'
  end
